...
def _build_template_flowset() -> bytes:
    fields = b"".join(struct.pack("!HH", t, length) for t, length in TEMPLATE_FIELDS)
    body = struct.pack("!HH", TEMPLATE_ID, len(TEMPLATE_FIELDS)) + fields
    return struct.pack("!HH", 0, 4 + len(body)) + body  # flowset_id 0 = template


def _build_record(idx: int, dev, uptime_ms: int, interval_ms: int) -> bytes:
    ip, mac, _hostname, rx_peak, tx_peak = dev
    peer = PUBLIC_PEERS[idx % len(PUBLIC_PEERS)]   # stable per device
...
def _build_packet(seq: int, uptime_ms: int, interval_ms: int) -> bytes:
    records = b"".join(
        _build_record(i, d, uptime_ms, interval_ms) for i, d in enumerate(DEVICES)
    )
    data_flowset = struct.pack("!HH", TEMPLATE_ID, 4 + len(records)) + records
    header = struct.pack(
        "!HHIIII",
        9,                        # version
    ...
    # Template before data so the collector has it when it reaches the data set.
    return header + _build_template_flowset() + data_flowset
...
def main() -> None:
    ...
    try:
        while True:
            ...
            sock.sendto(_build_packet(seq, uptime_ms, interval_ms), (args.host, args.port))
            seq += 1
